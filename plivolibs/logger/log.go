...
func generatePrefix(ctx config.TContext) string {
	return ctx.Name
}

func generateTrackingIDs(ctx config.TContext) string {
...
func GenericError(ctx config.TContext, e error, fields FieldsMap) {
...
func GenericInfo(ctx config.TContext, infoMessage string, fields FieldsMap) {
	prefix := generatePrefix(ctx)
	trackingIDs := generateTrackingIDs(ctx)
	fieldsBytes, _ := json.Marshal(fields)
	fieldsString := string(fieldsBytes)
...
func GenericWarning(ctx config.TContext, warnMessage string, fields FieldsMap) {
...
func GenericTrace(ctx config.TContext, traceMessage string, fields FieldsMap) {
